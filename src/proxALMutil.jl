...
function update_runinfo(runinfo::ProxALMData, opfdata::OPFData;
    ...
    push!(runinfo.objvalue,
        compute_objective_function(runinfo.x, opfdata; modelinfo = modelinfo)
    )
    push!(runinfo.lyapunov,
        compute_lyapunov_function(runinfo.x, runinfo.λ, opfdata; xref = runinfo.xprev, modelinfo = modelinfo, algparams = algparams)
    )
    ...
    if !isempty(runinfo.opt_sol)
        ...
        runinfo.dist_x[iter] = dist(runinfo.x, xstar; modelinfo = modelinfo, algparams = algparams)
        runinfo.dist_λ[iter] = dist(runinfo.λ, λstar; modelinfo = modelinfo, algparams = algparams)
